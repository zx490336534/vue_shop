<template>
  <div>
    ...
    <el-card>
      ...
      <tree-table class="treeTable" :data="catelist" :columns="columns" :selection-type="false" :expand-type="false" show-index index-text="#" border :show-row-hover="false">
        ...
        <template slot="opt" slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEditCatDialog(scope.row)">编辑 </el-button>
          <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleteCat(scope.row.cat_id)">删除 </el-button>
        </template>
      </tree-table>
      <!--分页-->
      <el-pagination
    ...
    <!--编辑对话框-->
    <el-dialog title="编辑分类" :visible.sync="editCatDialogVisible" width="50%" @close="editCateDialogClosed">
      <!--编辑分类的表单-->
...
export default {
  ...
  data() {
    return {
      ...
      //添加分类表单的验证规则对象
      addCateFormRules: {
        cat_name: [{ required: true, message: '请输入分类名称', trigger: 'blue' }],
      },
      // 父级分类列表
      parentCateList: [],
      // 指定级联选择器的配置对象
